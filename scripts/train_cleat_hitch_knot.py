...
def main():
    ...
    data_path = "data/cleat_hitch_knot"
    if not Path(data_path).exists():
        print(f"Error: Dataset directory '{data_path}' not found.")
        print("Please run collect_cleat_hitch_data.py first to create the dataset.")
        sys.exit(1)
        
    # Load knot definition
    knot_def_path = "knot_definitions/cleat_hitch_knot.knot"
    ...
    print("\nTraining complete!")
    print(f"\nTo run the classifier API, use: python run_cleat_hitch_classifier.py")
